...
1. **CLI Mode** (`python main.py cli`): A direct terminal interface interacting with the `ImprovedRAG` core.
2. **WebApp Mode** (`python main.py web`): A Next.js frontend communicating with a FastAPI backend (`api.py`), which in turn interfaces with the `ImprovedRAG` core.
...
### Streaming to the UI
If running in WebApp mode, the `api.py` backend captures the LLM tokens as they are generated by Ollama. It parses the tokens in real-time, separating the `<thinking>` stream from the `<answer>` stream, and pushes them via Server-Sent Events (SSE) to the Next.js frontend. 

The frontend updates the UI dynamically, allowing the user to watch the model's internal reasoning process unfold alongside the final answer.

---

## 5. Feedback Loop
When the generation is complete, the user can vote "Correct" or "Not Correct" in the WebApp. This feedback is logged into the `conversations.db` SQLite database.

A background task recalculates the Bayesian feedback priors ($P_{feedback}$) for the specific chunk IDs cited in that answer. The next time a user asks a similar question, the `Hybrid Merge` (Stage 3.3) will mathematically favor or penalize those chunks based on this historical interaction.
