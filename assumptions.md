# Detailed Synthesis for Query: *summazie the assumptions made by google's file system in about 300-400 words and save it in a file named assumptions.md*
...
### Key Grounded Evidence & Excerpts

#### 1. `gfs.pdf` *(Relevance Score: 0.861)*
...
runningoninexpensivecommodity hardware, anditdelivers
...
ronment, bothcurrentandanticipated, thatreﬂectamarked
...
1. INTRODUCTION
We have designed and implemented the Google File Sys-
tem (GFS) to meet therapidly growing demandsof Google’s
...
has been driven bykey observations of our application work-
...
working with fast growing datasets of manyTBs comprising
...
ratherthanoverwritingexistingdata. Randomwriteswithin
...
repositoriesthatdataanalysisprogramsscanthrough. Some
...
atomic appendoperation sothatmultipleclientscan append
...
componentsthatoften fail. Itmustconstantlymonitor
...
doesnotimplementastandardAPIsuchasPOSIX.Filesare
organizedhierarchically indirectoriesandidentiﬁedbypath-
names. We support the usual operations tocreate, delete,
open,close,read,a n dwriteﬁles.
Moreover, GFS hassnapshotandrecord appendopera-
...
applications. Snapshotand record appendare discussed fur-
ther in Sections 3.4 and 3.3 respectively.
2.3 Architecture
A GFS cluster consists of a singlemasterand multiple
chunkserversand is accessed by multipleclients,a ss h o w n
in Figure 1. Each of these is typically a commodity Linux
machine runninga user-level server process. Itis easy to run
bothachunkserverandaclientonthesamemachine, aslong
as machine resources permit and the lower reliability caused
by running possibly ﬂaky application code is acceptable.
Files are divided into ﬁxed-sizechunks. Each chunkis
identiﬁed by an immutable and globally unique 64 bitchunk
handleassigned by the master at the time of chunkcreation.
Chunkservers store chunks on local disks as Linux ﬁles and
read or write chunkdata speciﬁed by a chunkhandle and
byterange. For reliability, each chunkis replicated on multi-
plechunkservers. By default, we store threereplicas, though
...
andsoLinux’sbuﬀercachealreadykeepsfrequentlyaccessed
data in memory.
2.4 Single Master
...
and write ﬁledata throughthemaster. Instead, a client asks
...
Letusexplaintheinteractionsforasimplereadwithrefer-
...
information using the ﬁle name and chunkindex as the k ey.
...
2.5 Chunk Size
Chunksize is one of the k ey design parameters. We have
...
Ontheotherhand,alargechunksize, evenwithlazyspace
allocation, has its disadvantages. A small ﬁle consists of a
small numberof chunks,perhapsjust one. The chunkservers
...
by a batch-queuesystem: an executable was written to GFS
...
and chunknamespaces, the mapping from ﬁles to chunk s,
and the locations of each chunk’s replicas. All metadata is
kept in the master’s memory. The ﬁrst two types (names-
paces and ﬁle-to-chunkmapping) are also k ept persistent by
...
Sincemetadataisstoredinmemory,masteroperationsare
...
chunk. Most chunksare full because mostﬁles containmany
...
chunkplacement and monitors chunk server status with reg-
ularHeartBeatmessages.
Weinitially attempted tokeep chunklocation information
...
keepingthemasterand chunkserversin syncas chunkservers
...
records together before ﬂushingtherebyreducing theimpact
...
log small. The master checkpoints its state wheneverthe log
...
rizes the result. A ﬁle region isconsistentif all clients will
always see the same data, regardless of which replicas they
read from. A region isdeﬁnedafter a ﬁle data mutation if it
...
Data mutations may bewritesorrecord appends.Aw r i t e
...
from validreplicasassoon aspossible(Section4.3). Achunk
...
tency model with a few simple techniquesalready needed for
...
queue. Recordappend’sappend-at-least-oncesemanticspre-
...
implementdatamutations, atomicrecordappend,andsnap-
shot.
3.1 Leases and Mutation Order
A mutation is an operation that changes the contents or
metadata of a chunksuch as a write or an append opera-
tion. Each mutation is performed at all the chunk’sreplicas.
Weuseleases to maintain aconsistent mutation order across
...
2. Themasterreplieswiththeidentityoftheprimaryand
...
worktopology regardless of which chunk server is the
...
primary andanarbitrary subsetof thesecondary repli-
...
clients, although thereplicas will beidenticalbecause thein-
...
pushedlinearlyalongacarefullypickedchainofchunkservers
in a pipelined fashion. Our goals are to fully utilize each
machine’s networkbandwidth, avoid networkbottleneck s
...
fer overTCP connections. Oncea chunkserverreceives some
...
ideal elapsed time for transferringB bytes toR replicas is
B/T + RL whereT is the networkthroughput andL is la-
tency to transfer bytes between two machines. Our network
links are typically 100 Mbps (T), andL is far below 1 ms.
...
most one-fourth of the maximum chunksize to k eep worst-
...
Ifarecord appendfails atanyreplica, theclient retriesthe
...
operation toreport success, thedatamust havebeenwritten
...
later becomes theprimary. Interms of ourconsistency guar-
...
Like AFS[5], we use standardcopy-on-write techniquesto
implement snapshots. When the master receives a snapshot
request, it ﬁrst revokes any outstanding leases on thechunks
in the ﬁles it is about to snapshot. This ensures that any
subsequentwrites to these chunkswill require an interaction
...
ple, asnapshotoperationhastorevokechunkserverleases on
all chunkscoveredbythesnapshot. Wedo notwantto delay
...
logically representsits namespaceas alookuptable mapping
...
full pathname/d1/d2/.../dn/leaf.N o t et h a tleafmay be
a ﬁle or directory depending on the operation.
Wenowillustrate howthislockingmechanism can prevent
aﬁ l e/home/user/foo from being created while/home/user
is being snapshotted to/save/user. The snapshot oper-
a t i o na c q u i r e sr e a dl o c kso n/home and/save,a n dw r i t e
...
name twice.
Sincethenamespacecanhavemanynodes, read-writelock
...
traﬃc hastoﬂowthrough multipleracks, atradeoﬀ we make
willingly.
...
tors. (1)Wewant toplacenewreplicason chunkserverswith
below-average diskspace utilization. Over time this will
equalize diskutilization across chunkservers. (2) Wewant to
limit the number of “recent” creations on each chunkserver.
Although creation itself is cheap, it reliably predicts immi-
nentheavywrite traﬃc because chunksare createdwhende-
mandedby writes, andin ourappend-once-read-manywork-
load they typically become practically read-only once they
have been completely written. (3) As discussed above, we
want to spread replicas of a chunkacross rack s.
The masterre-replicatesa chunkas soon as the number
...
priority of any chunkthat is block ing client progress.
...
Linuxﬁles underdesignateddirectories oneachchunkserver.
...
sages maybelost, and themaster hastoremembertoresend
...
Chunkreplicas may become stale if a chunk server fails
...
in a cloning operation. The client or the chunkserververiﬁes
...
ponentfailures can result inan unavailablesystem or, worse,
corrupted data. We discuss how we meet these challenges
andthetools wehavebuilt intothesystemtodiagnose prob-
...
such as garbage collection that change thesystem internally.
...
the ﬁle system even when theprimary master is down. They
...
5.2 Data Integrity
Eachchunkserveruseschecksummingtodetectcorruption
ofstoreddata. GiventhataGFSclusteroftenhasthousands
...
taining checksums.
Achunkis brokenupinto64 KBblocks. Each hasacorre-
...
value will not match thestored data, and thecorruption will
...
partially, thenewchecksumsmayhidecorruptionthatexists
...
monitoring.
6. MEASUREMENTS
Inthissection wepresent afewmicro-benchmarkstoillus-
...
Figure 3(a) shows the aggregate read rate forN clients
...
rate reaches94 MB/s, about 75% of the125 MB/s linklimit,
for 16 readers, or 6 MB/s per client. The eﬃciency drops
from 80% to75% becauseasthenumberofreadersincreases,
so does the probability that multiple readers simultaneously
read from the same chunkserver.
6.1.2 Writes
N clients write simultaneously toN distinct ﬁles. Each
...
c a u s ew en e e dt ow r i t ee a c hb y t et o3o ft h e1 6c h u n ks e r v e r s ,
...
thecaseof reads, itbecomes morelikely thatmultipleclients
...
ited by the networkbandwidth of the chunk servers that
...
currently. In other words,N clients append toM shared
ﬁles simultaneously where bothN andM are in the dozens
or hundreds. Therefore, the chunkservernetwork congestion
...
longer and continuously generate and process multi-TBdata
sets with only occasional human intervention. In both cases,
asingle “task” consists of manyprocesses on manymachines
reading and writing many ﬁles simultaneously.
6.2.1 Storage
Asshownbytheﬁrstﬁveentriesinthetable, bothclusters
...
middleofaburstofwriteactivitygeneratingabout100MB/s
...
Read rate (since restart)
 589 MB/s
 49 MB/s
W rite rate (last minute)
 1M B / s
 101 MB/s
W rite rate (last hour)
 2M B / s
 117 MB/s
W r i t er a t e( s i n c er e s t a r t )
 25 MB/s
 13 MB/s
...
not a bottleneckfor these work loads.
In an earlier version of GFS, the master was occasionally
a bottleneckfor some work loads. It spent most of its time
...
pactonrunningapplications andprovideleeway forschedul-
...
6.25 MB/s (50 Mbps). All chunkswere restored in 23.2 min-
utes, at an eﬀective replication rate of 440 MB/s.
In another experiment, we killed two chunkservers each
withroughly16,000chunksand660GBofdata. Thisdouble
...
mutualinﬂuencemayalso existbetweengeneralapplications
Operation
 Read
 Wr i te
 Record Append
...
reads, thesize is theamount of data actually read and trans-
...
ﬁles.
A signiﬁcant numberof reads returnno data at all in clus-
...
pointorsynchronizemoreoften, orsimply generateless data
...
cause of the random seekwork load.
...
clusterY,theratiosareboth0.05%. Althoughthisisminute,
...
diﬀerence inOpenrequests because an old version of a ﬁle
may be implicitly deleted by being opened for write from
scratch (mode “w” in Unix open terminology).
FindMatchingFilesisapatternmatchingrequestthatsup-
...
ruption, while concurrentlywe modiﬁed thekernelto handle
...
tually, we found that this single lockblock ed the primary
...
Despiteoccasional problems, theavailabilityofLinuxcode
...
fault tolerance. UnlikeAFS,GFS spreads aﬁle’s dataacross
...
nesota’sGFS[11]andGPFS[10]removethecentralizedserver
...
to implement sophisticated chunkplacement andreplication
...
diskdrives, GFS uses commodity machines as chunk servers,
as done in the NASD prototype. Unlike the NASD work,
ourchunkserversuselazily allocated ﬁxed-sizechunksrather
...
tributedqueuesbutlacksthefaulttolerancethatcomeswith
...
Westarted byreexamining traditional ﬁlesystem assump-
...
covery. Chunkreplication allows us to tolerate chunk server
failures. The frequency of these failures motivated a novel
onlinerepairmechanism thatregularly andtransparentlyre-
...
concurrentreaders and writers performing a variety of tasks.
...
Serverless networkﬁle systems. InProceedings of the
...
Replication in the Harp ﬁle system. In13th
...
[10] FrankSchmuckand Roger Hask in. GPFS: A
...
Across the **1** retrieved documents, the concepts above are linked through multi-level hybrid indexing and verified against the document repository.